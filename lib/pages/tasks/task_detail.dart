...
class _TaskDetailState extends State<TaskDetail> {
      ...
      return Scaffold(
        ...
        body: BlocBuilder<TasksBloc, TasksState>(builder: (context, taskState) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            _checkIfTaskIsConflicted(context, taskState);
          });
  ...
  _checkIfTaskIsConflicted(BuildContext context, TasksState taskState) async {
      ...
      if (conflicts != null && conflicts.isNotEmpty) {
        final result = await showDialog(
            ...
            builder: (context) => ABModal(
                  title: context.t.tasks.conflict_detected_modal.title,
                  description:
                      context.t.tasks.conflict_detected_modal.description,
                  warning: context.t.tasks.conflict_detected_modal.warning,
                  confirmText:
                      context.t.tasks.conflict_detected_modal.resolve_now,
                  confirmColor: getTheme(context).primary,
                  cancelText: context.t.tasks.conflict_detected_modal.later,
